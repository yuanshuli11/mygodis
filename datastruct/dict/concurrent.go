...
func (dict *ConcurrentDict) getShardByHash(hashCode uint32) *Shard {
	if dict == nil {
		panic("dict is nil")
	}
	return dict.table[(uint32(len(dict.table)) - 1) & uint32(hashCode)]
}
...
func (dict *ConcurrentDict) Get(key string) (val interface{}, exists bool) {
	...
	hashCode := fnv32(key)
	shard := dict.getShardByHash(hashCode)
	shard.mutex.RLock()
